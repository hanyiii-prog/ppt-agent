...
def test_clone_build_rejects_bad_pages(tmp_path):
    from ppt_agent.mcp.tools import call_tool

    context = _context(tmp_path)
    tpl = str(tmp_path / "tpl.pptx")
    _write_template(tpl)
    result = call_tool("ppt_agent_clone_build",
                       {"template": tpl, "pages": [], "output": "o.pptx"}, context)
    assert result["isError"] is True
    assert "pages" in result["content"][0]["text"]


def _write_template(path):
    """Same fixture as mini_template, materialised for the MCP-layer tests."""
    prs = Presentation()
    l0 = prs.slide_layouts[0]
    l2 = prs.slide_layouts[2]
    l5 = prs.slide_layouts[5]
